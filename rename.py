...
def rename_files():
    ...
    try:
        ...
        # 画像として一般的な拡張子を対象にする（必要に応じて追加してください）
        valid_extensions = (
            ".jpg",
            ".jpeg",
            ".png",
        ...
        # 画像ファイルだけを抽出
        image_files = [
            f for f in files if os.path.endswith(f.lower()) or f.endswith(valid_extensions)
        ]
        ...
        for index, filename in enumerate(image_files, start=1):
            ...
            # 新しいファイル名を作成 (例: 指定名1.jpg, 指定名2.jpg ...)
            ...
            if os.path.exists(new_file_path):
                # 一時的に別名にするなどの処理も考えられますが、
                # 今回は安全のためスキップするか警告を出します
                continue
...
entry_folder.pack(side="left", fill="x", expand=True, mr=5)
